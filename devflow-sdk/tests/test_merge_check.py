...
class TestIsAncestor(unittest.TestCase):
    def test_merged_branch_returns_true(self):
        ...
        with patch("devflow_sdk.core.git.merge_check.subprocess.run", mock_run):
    ...
    def test_unmerged_branch_returns_false(self):
        with patch("devflow_sdk.core.git.merge_check.subprocess.run", return_value=MagicMock(returncode=1)):
            self.assertFalse(_is_ancestor("/repo", "feat/33-add-x", "origin/main"))

    def test_unexpected_error_exits(self):
        with patch(
            "devflow_sdk.core.git.merge_check.subprocess.run",
...
class TestCheckGhMergedPr(unittest.TestCase):
    ...
    def test_gh_not_installed_returns_none(self):
        with patch("devflow_sdk.core.git.merge_check.subprocess.run", side_effect=FileNotFoundError()):
...
class TestPatchIdMatches(unittest.TestCase):
    def test_matching_commit_returns_true(self):
        ...
        with patch("devflow_sdk.core.git.merge_check.subprocess.run", side_effect=side_effect):
...
class TestIsMerged(unittest.TestCase):
    def test_ancestor_true_short_circuits(self):
        with patch("devflow_sdk.core.git.merge_check.subprocess.run", return_value=MagicMock(returncode=0)), \
             patch("devflow_sdk.core.git.merge_check._is_ancestor", return_value=True) as mock_ancestor, \
             patch("devflow_sdk.core.git.merge_check._check_gh_merged_pr") as mock_gh, \
             patch("devflow_sdk.core.git.merge_check._patch_id_matches") as mock_patch_id:
    ...
    def test_gh_true_short_circuits_patch_id(self):
        with patch("devflow_sdk.core.git.merge_check.subprocess.run", return_value=MagicMock(returncode=0)), \
             patch("devflow_sdk.core.git.merge_check._is_ancestor", return_value=False) as mock_ancestor, \
             patch("devflow_sdk.core.git.merge_check._check_gh_merged_pr", return_value=True) as mock_gh, \
             patch("devflow_sdk.core.git.merge_check._patch_id_matches") as mock_patch_id:
            self.assertTrue(is_merged("/repo", "feat/33-add-x", "main"))
        mock_ancestor.assert_called_once()
    ...
    def test_gh_inconclusive_falls_back_to_patch_id(self):
        with patch("devflow_sdk.core.git.merge_check.subprocess.run", return_value=MagicMock(returncode=0)), \
             patch("devflow_sdk.core.git.merge_check._is_ancestor", return_value=False), \
             patch("devflow_sdk.core.git.merge_check._check_gh_merged_pr", return_value=None), \
             patch("devflow_sdk.core.git.merge_check._patch_id_matches", return_value=True) as mock_patch_id:
            self.assertTrue(is_merged("/repo", "feat/33-add-x", "main"))
    ...
    def test_gh_false_falls_back_to_patch_id(self):
        with patch("devflow_sdk.core.git.merge_check.subprocess.run", return_value=MagicMock(returncode=0)), \
             patch("devflow_sdk.core.git.merge_check._is_ancestor", return_value=False), \
             patch("devflow_sdk.core.git.merge_check._check_gh_merged_pr", return_value=False), \
             patch("devflow_sdk.core.git.merge_check._patch_id_matches", return_value=True) as mock_patch_id:
            self.assertTrue(is_merged("/repo", "feat/33-add-x", "main"))
        mock_patch_id.assert_called_once()
    ...
    def test_all_tiers_false_returns_false(self):
        with patch("devflow_sdk.core.git.merge_check.subprocess.run", return_value=MagicMock(returncode=0)), \
             patch("devflow_sdk.core.git.merge_check._is_ancestor", return_value=False), \
             patch("devflow_sdk.core.git.merge_check._check_gh_merged_pr", return_value=False), \
             patch("devflow_sdk.core.git.merge_check._patch_id_matches", return_value=False):
